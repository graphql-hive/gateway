  ...
  return function getExtraPossibleTypes(typeName: string) {
    ...
    if (!extraTypesForSubschema) {
      ...
      if (isAbstractType(gatewayType) && isAbstractType(subschemaType)) {
        ...
        const possibleTypesInSubschema =
          transformedSchema.getPossibleTypes(subschemaType);
      ...
      if (selection.kind === Kind.INLINE_FRAGMENT) {
        if (selection.typeCondition != null) {
          ...
          if (!visitedSelections.has(selection)) {
            ...
            const getExtraPossibleTypes = getExtraPossibleTypesFn(
              transformedSchema,
              infoSchema,
            );
            ...
            if (
              isObjectType(typeInSubschema) ||
              isInterfaceType(typeInSubschema)
            ) {
          ...
          for (const possibleTypeName of possibleTypes) {
            const maybePossibleType =
              transformedSchema.getType(possibleTypeName);
            if (
...
const getSchemaMetaData = memoize2(
  (
    ...
    const interfaceExtensionsMap: Record<
      string,
    ...
    const stitchingInfo = sourceSchema.extensions?.[
      'stitchingInfo'
    ] as StitchingInfo;
    return {
      possibleTypesMap,
...
function wrapConcreteTypes(
  ...
  let possibleTypes: readonly GraphQLNamedOutputType[] = isAbstractType(
    namedType,
  )
