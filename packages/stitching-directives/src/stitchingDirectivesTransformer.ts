...
export function stitchingDirectivesTransformer(
  ...
  return (subschemaConfig: SubschemaConfig): SubschemaConfig => {
    ...
    const selectionSetsByType: Record<string, SelectionSetNode> =
      Object.create(null);
    const computedFieldSelectionSets: Record<
      string,
      Record<string, SelectionSetNode>
    > = Object.create(null);
    const mergedTypesResolversInfo: Record<string, MergedTypeResolverInfo> =
      Object.create(null);
    ...
    function setCanonicalDefinition(
      typeName: string,
      fieldName?: string,
    ): void {
      canonicalTypesInfo[typeName] =
        canonicalTypesInfo[typeName] || Object.create(null);
      if (fieldName) {
        ...
        if (mergeDirective?.['keyField'] != null) {
          ...
          forEachConcreteType(schema, returnType, typeNames, (typeName) => {
            if (typeNames == null || typeNames.includes(typeName)) {
              let existingEntryFieldMap =
                selectionSetsByTypeAndEntryField[typeName]!;
              if (existingEntryFieldMap == null) {
                existingEntryFieldMap = Object.create(null);
                selectionSetsByTypeAndEntryField[typeName] =
                  existingEntryFieldMap;
              }
    ...
    const allSelectionSetsByType: Record<
      string,
      Array<SelectionSetNode>
    > = Object.create(null);
    ...
    for (const typeName in computedFieldSelectionSets) {
      ...
      for (const i in selectionSets) {
        allSelectionSetsByType[typeName] =
          allSelectionSetsByType[typeName] || [];
    ...
    mapSchema(schema, {
      [MapperKind.OBJECT_FIELD]: function objectFieldMapper(
        fieldConfig,
        fieldName,
      ) {
        const mergeDirective = getDirective(
        ...
        if (mergeDirective != null) {
          ...
          if (mergeArgsExpr == null) {
            ...
            const argNames =
              keyArg == null
                ? [Object.keys(fieldConfig.args ?? {})[0]]
                : keyArg.split('.');
          ...
          forEachConcreteTypeName(
            ...
            function generateResolveInfo(typeName) {
              ...
              if (
                selectionSetsByTypeAndEntryField[typeName]?.[fieldName] != null
              ) {
                const typeConfigByField = (mergedTypesResolversInfoByEntryField[
                  typeName
                ] ||= Object.create(null));
    ...
    for (const typeName in mergedTypesResolversInfoByEntryField) {
      ...
      for (const fieldName in mergedTypesResolversInfoByEntryField[typeName]) {
        const mergedTypeResolverInfo =
          mergedTypesResolversInfoByEntryField[typeName][fieldName]!;
...
function buildKeyExpr(key: Array<string>): string {
  ...
  for (const keyDef of key) {
    let [aliasOrKeyPath, keyPath] = keyDef.split(':') as [
      string,
      string | undefined,
    ];
...
function mergeSelectionSets(
  ...
  const normalizedSelections: Record<string, SelectionNode> =
    Object.create(null);
