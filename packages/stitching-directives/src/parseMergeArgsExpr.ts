...
export function parseMergeArgsExpr(
  ...
  const { inputValue: newInputValue, variablePaths } =
    extractVariables(inputValue);
  ...
  if (!Object.keys(expansionExpressions).length) {
    ...
    const usedProperties = propertyTreeFromPaths(
...
function getMappingInstructions(
  variablePaths: VariablePaths,
): Array<MappingInstruction> {
