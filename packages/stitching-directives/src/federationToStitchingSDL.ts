...
function getQueryTypeDef(
  ...
  const typeName = schemaDef
    ? schemaDef.operationTypes.find(({ operation }) => operation === 'query')
        ?.type.name.value
    : 'Query';
...
export function federationToStitchingSDL(
  ...
  doc.definitions.forEach((typeDef) => {
    ...
    if (
      ...
      (typeDef.kind as string) = typeDef.kind.replace(
        extensionKind,
        'Definition',
      );
    }
    ...
    const keyFields = (
      parse(selectionSet).definitions[0] as any
    ).selectionSet.selections.map((sel: any) => sel.name.value);
