...
describe('type merging directives', () => {
  ...
  test('adds type selection sets', () => {
    ...
    const transformedSubschemaConfig =
    ...
    expect(transformedSubschemaConfig.merge?.['User']?.fieldName).toEqual(
      '_user',
    );
  });
  ...
  test('adds computed selection sets', () => {
    ...
    expect(
      transformedSubschemaConfig.merge?.['User']?.fields?.['name']
        ?.selectionSet,
    ).toEqual(print(parseSelectionSet('{ id }')));
    expect(
      transformedSubschemaConfig.merge?.['User']?.fields?.['name']?.computed,
    ).toEqual(true);
    expect(transformedSubschemaConfig.merge?.['User']?.fieldName).toEqual(
      '_user',
    );
  ...
  test('adds args function when used with key argument', () => {
    const typeDefs = /* GraphQL */ `
      ...
      type Query {
        _user(key: _Key): User
          @merge(key: ["id", "outer.inner.firstName:name.firstName"])
      }
  ...
  test('adds key and argsFromKeys functions when used without arguments', () => {
    ...
    const argsFromKeysFn =
      transformedSubschemaConfig.merge?.['User']?.argsFromKeys!;
