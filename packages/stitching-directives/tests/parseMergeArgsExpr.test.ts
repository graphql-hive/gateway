...
describe('can parse merge arguments', () => {
  ...
  test('can parseMergeArgsExpr with key', () => {
    ...
    expect(result).toEqual({
      ...
      mappingInstructions: [
        { destinationPath: ['test'], sourcePath: ['test'] },
      ],
    });
  });
  ...
  test('can parseMergeArgsExpr with nested key', () => {
    ...
    expect(result).toEqual({
      ...
      usedProperties: { test: null },
      mappingInstructions: [
        { destinationPath: ['outer', 'inner'], sourcePath: ['test'] },
      ],
    });
  });
  ...
  test('can parseMergeArgsExpr with complex key', () => {
    ...
    expect(result).toEqual({
      ...
      mappingInstructions: [
        { destinationPath: ['test'], sourcePath: ['outer', 'inner'] },
      ],
    });
  });
  ...
  test('can parseMergeArgsExpr with key defined by selectionSet', () => {
    const selectionSet = parseSelectionSet(
      `{ field1 field2 { subFieldA subFieldB } }`,
    );
    const args = `test: $key`;
      ...
      expansions: [
        {
          ...
          value: { inner: null },
          mappingInstructions: [
            { destinationPath: ['inner'], sourcePath: ['test'] },
          ],
        },
      ],
    });
      ...
      expansions: [
        {
          ...
          value: null,
          mappingInstructions: [
            { destinationPath: [], sourcePath: ['outer', 'inner'] },
          ],
        },
      ],
    });
