// straight up copied from https://github.com/modelcontextprotocol/typescript-sdk/blob/ab552c30d5499b5e0aaa1ab47ee2481028910c3d/packages/core/src/util/standardSchema.ts
// because @modelcontextprotocol/core is not published (is a private package)
 ...
 */
export interface StandardSchemaV1Sync<
  Input = unknown,
  Output = Input,
...
export async function validateStandardSchema<T extends StandardSchemaV1>(
  ...
  return {
    success: true,
    data: (result as StandardSchemaV1.SuccessResult<unknown>)
      .value as StandardSchemaV1.InferOutput<T>,
  };
}
...
export function promptArgumentsFromStandardSchema(
  ...
  const properties =
    (jsonSchema['properties'] as Record<string, { description?: string }>) ||
    {};
