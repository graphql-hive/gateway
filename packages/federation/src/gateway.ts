...
export function getSubschemaForFederationWithTypeDefs(
  typeDefs: DocumentNode,
): SubschemaConfig {
  const subschemaConfig = {} as SubschemaConfig;
  const typeMergingConfig = (subschemaConfig.merge =
        ...
        for (const fieldNode of node.fields) {
          ...
          if (fieldNode.directives) {
            ...
            for (const directive of fieldNode.directives) {
              ...
              switch (directive.name.value) {
                case 'requires': {
                  const typeMergingFieldsConfig =
                    (typeMergingTypeConfig.fields =
                      typeMergingTypeConfig.fields || {});
                  typeMergingFieldsConfig[fieldName] =
                    typeMergingFieldsConfig[fieldName] || {};
                  if (
                    directiveArgs.some(
                      (arg) =>
...
export async function getSubschemaForFederationWithExecutor(
  executor: Executor,
) {
...
export async function getSubschemaForFederationWithSchema(
...
export async function getStitchedSchemaWithUrls(
...
export const federationSubschemaTransformer: SubschemaConfigTransform =
  function federationSubschemaTransformer(
