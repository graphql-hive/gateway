...
describe('merge canonical types', () => {
  ...
  it('merges prioritized descriptions', () => {
    ...
    expect(gatewaySchema.getType('ProductScalar')?.description).toEqual(
      'first',
    );
  ...
  it('merges prioritized ASTs', () => {
    ...
    expect(
      getDirective(firstSchema, queryType.toConfig(), 'mydir')?.[0]?.['value'],
    ).toEqual('first');
    expect(
      getDirective(firstSchema, objectType.toConfig(), 'mydir')?.[0]?.['value'],
    ).toEqual('first');
    expect(
      getDirective(firstSchema, interfaceType.toConfig(), 'mydir')?.[0]?.[
        'value'
      ],
    ).toEqual('first');
    expect(
      getDirective(firstSchema, inputType.toConfig(), 'mydir')?.[0]?.['value'],
    ).toEqual('first');
    expect(
      getDirective(firstSchema, enumType.toConfig(), 'mydir')?.[0]?.['value'],
    ).toEqual('first');
    expect(
      getDirective(firstSchema, unionType.toConfig(), 'mydir')?.[0]?.['value'],
    ).toEqual('first');
    expect(
      getDirective(firstSchema, scalarType.toConfig(), 'mydir')?.[0]?.['value'],
    ).toEqual('first');

    expect(
      getDirective(
        firstSchema,
        queryType.getFields()['field1']!,
        'mydir',
      )?.[0]?.['value'],
    ).toEqual('first');
    expect(
      getDirective(
        firstSchema,
        queryType.getFields()['field2']!,
        'mydir',
      )?.[0]?.['value'],
    ).toEqual('second');
    expect(
      getDirective(firstSchema, objectType.getFields()['id']!, 'mydir')?.[0]?.[
        'value'
      ],
    ).toEqual('first');
    expect(
      getDirective(firstSchema, objectType.getFields()['url']!, 'mydir')?.[0]?.[
        'value'
      ],
    ).toEqual('second');
    expect(
    ...
    expect(
      getDirective(
        firstSchema,
        interfaceType.getFields()['url']!,
        'mydir',
      )?.[0]?.['value'],
    ).toEqual('second');
    expect(
      getDirective(firstSchema, inputType.getFields()['id']!, 'mydir')?.[0]?.[
        'value'
      ],
    ).toEqual('first');
    expect(
      getDirective(firstSchema, inputType.getFields()['url']!, 'mydir')?.[0]?.[
        'value'
      ],
    ).toEqual('second');

    expect(
      enumType.toConfig().astNode?.values?.map((v) => v.description?.value),
    ).toEqual(['first', 'first', 'second']);
    expect(
      enumType.toConfig().values['YES']?.astNode?.description?.value,
    ).toEqual('first');
    expect(
      enumType.toConfig().values['NO']?.astNode?.description?.value,
    ).toEqual('first');
    expect(
      enumType.toConfig().values['MAYBE']?.astNode?.description?.value,
    ).toEqual('second');
  });
