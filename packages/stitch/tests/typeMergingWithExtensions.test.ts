...
describe('merging using type merging', () => {
  const { stitchingDirectivesValidator, stitchingDirectivesTransformer } =
    stitchingDirectives();
        ...
        args: {
          _keys: {
            type: new GraphQLNonNull(
              new GraphQLList(new GraphQLNonNull(accountsSchemaTypes._Key)),
            ),
          },
        },
        resolve: (_root, { keys }) =>
          keys.map((key: Record<string, any>) =>
            users.find((u) => u.id === key['id']),
          ),
        extensions: {
          directives: [
            {
      ...
      _products: {
        ...
        args: {
          keys: {
            type: new GraphQLNonNull(
              new GraphQLList(
                new GraphQLNonNull(inventorySchemaTypes.ProductKey),
              ),
            ),
          },
        },
  ...
  reviewsSchemaTypes.Product = new GraphQLObjectType({
    ...
    fields: () => ({
      ...
      reviews: {
        type: new GraphQLList(reviewsSchemaTypes.Review),
        resolve: (product) =>
          reviews.filter((review) => review.product.upc === product.upc),
      },
    }),
    extensions: {
      directives: {
