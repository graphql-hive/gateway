...
describe('test merged composite computed fields', () => {
  ...
  describe('object-valued computed field', () => {
    const schemaB = makeExecutableSchema({
      ...
      resolvers: {
        T: {
          next: (obj: { id: string; value: number }) => ({
            id: `${obj.value + 1}`,
          }),
        ...
        Query: {
          ...
          uByRepresentation: (
            _: never,
            { representation: { id } }: { representation: { id: string } },
          ) => ({ id }),
        },
      },
    });
        ...
        },
        {
          ...
          merge: {
            T: {
              selectionSet: '{ id }',
              fieldName: 'byRepresentation',
              args: ({ id, value }: { id: string; value?: number }) => ({
                representation: { id, value },
              }),
              fields: {
                next: {
