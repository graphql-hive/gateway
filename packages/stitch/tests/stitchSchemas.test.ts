...
bookingById(id: "b1") {
  ...
  `;
      ...
      test('interfaces', async () => {
        ...
        const mergedDelegate = await graphql({
          schema: stitchedSchema,
          source: delegateQuery,
        });
...
operationNameTest {
  ...
  `;
        ...
        expect(localResult?.data?.operationNameTest?.testString).toBe(
          localOperationName,
        );
        ...
        expect(stitchedResult?.data?.operationNameTest?.testString).toBe(
          stitchedOperationName,
        );
      });
    });
    ...
    describe('aliases', () => {
      test('should allow aliasing on the gateway level in a complex schema with new types', async () => {
                  ...
                  transforms: [
                    {
                      transformRequest: (ast) => {
                        ...
                        const pageNode =
                          personaNode.selectionSet.selections.find(
                      ...
                      transformResult: (originalResult: ExecutionResult) => {
                        ...
                        originalResult.data['persona'] = {
                          page: originalResult.data['persona'].transactions
                            .items,
                        };
        ...
        const flattenedTransactionsData: any =
          result.data['flattenedTransactions'];
