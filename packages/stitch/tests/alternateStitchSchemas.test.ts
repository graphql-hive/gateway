...
describe('merge schemas through transforms', () => {
  ...
  test('local subscriptions should work even if root fields are renamed', async () => {
    ...
    expect(await payload).toEqual({
      ...
      value: { data: transformedNotification },
    });
  });
});
...
describe('default values', () => {
  test('should work to add a default value even when renaming root fields', async () => {
    const transformedPropertySchema = wrapSchema({
      schema: propertySchema,
      transforms: [
        new TransformRootFields((typeName, fieldName, fieldConfig): any => {
...
describe('rename fields that implement interface fields', () => {
  test('should work', () => {
    ...
    const originalResult = graphqlSync({
      schema: originalSchema,
...
describe('rename nested object fields with interfaces', () => {
  test('should work', () => {
    ...
    const transformedResult = graphqlSync({
...
describe('WrapType', () => {
  ...
  test('Subscription transform should work', async () => {
    const transformedSchema = wrapSchema({
      ...
      transforms: [
        new WrapType('Subscription', 'Namespace_Subscription', 'namespace'),
      ],
    });
...
describe('HoistField transform', () => {
  ...
  test('should work to hoist fields without using arguments', async () => {
    const wrappedSchema = wrapSchema({
      schema,
      transforms: [
        new HoistField('Outer', ['inner', 'test'], 'hoisted'),
        new PruneSchema({}),
      ],
    });
  ...
  test('should work to hoist fields with using arguments', async () => {
    ...
    const result = await graphql({
      schema: wrappedSchema,
      source: '{ query { hoisted(testArg: "custom") } }',
    });
  ...
  test('should work to hoist fields with using arguments from fields in path', async () => {
    const wrappedSchema = wrapSchema({
      ...
      transforms: [
        new HoistField(
          'Outer',
          [{ fieldName: 'inner', argFilter: () => true }, 'test'],
          'hoisted',
        ),
        new PruneSchema({}),
      ],
  ...
  test('should work to hoist fields to new root fields', async () => {
    const wrappedSchema = wrapSchema({
      schema,
      transforms: [
        new HoistField('Query', ['query', 'inner', 'test'], 'hoisted'),
        new PruneSchema({}),
      ],
    });
  ...
  it('should work to hoist fields in arrays', async () => {
    const wrappedSchema = wrapSchema({
      schema,
      transforms: [
        new HoistField('User', ['friends', 'entries', 'user'], 'friendUsers'),
        new PruneSchema({}),
      ],
    });
  ...
  test('returns info.left and info.right properties that are not equal', () => {
    stitchSchemas({
      ...
      onTypeConflict: (left, _right, info) => {
        ...
        expect(
          info?.right.transformedSubschema !== info?.left.transformedSubschema,
        ).toBe(true);
        return left;
      },
    });
