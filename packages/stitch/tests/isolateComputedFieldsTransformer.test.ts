...
describe('isolateComputedFieldsTransformer', () => {
  describe('basic isolation', () => {
    ...
    it('splits a subschema into base and computed portions', async () => {
      ...
      expect(
        baseSubschema.transformedSchema.getType('DeliveryService'),
      ).toBeDefined();
      expect(
        baseSubschema.transformedSchema.getType('Storefront'),
      ).toBeDefined();
      expect(
        baseSubschema.transformedSchema.getType('ProductRepresentation'),
      ).toBeDefined();

      expect(
        Object.keys(
          (
            computedSubschema.transformedSchema.getType(
              'Query',
            ) as GraphQLObjectType
          ).getFields(),
        ),
...
type Mutation {
  addProduct(name: String!): Product!
    ...
    it.each([
      ...
      'does not isolate $name referenced from other fields',
      async ({ variant }) => {
        const [computedConfig, baseConfig] = isolateComputedFieldsTransformer({
  ...
  describe('with composite return type', () => {
    ...
    it.skip('return type is unmerged type', () => {
      ...
      const baseQueryType = baseSchema.transformedSchema.getType(
        'Query',
      ) as GraphQLObjectType;
    ...
    it('return type is merged type', () => {
      ...
      const baseQueryType = baseSubschema.transformedSchema.getType(
        'Query',
      ) as GraphQLObjectType;
      const baseQueryTypeFields = baseQueryType.getFields();
      expect(baseQueryTypeFields['_item']).toBeDefined();
