      ...
      expect(() => {
        stitchSchemas({
          ...
          subschemas: [
            {
              schema: buildSchema(
                'type Query { field: [Thing] } type Thing { id: ID }',
              ),
            },
            { schema: buildSchema('type Query { field: [ID] }') },
          ],
      ...
      expect(() => {
        stitchSchemas({
          ...
          subschemas: [
            ...
            {
              schema: buildSchema(
                'type Query { field(arg: URL): String } scalar URL',
              ),
            },
          ],
        });
      ...
      expect(() => {
        stitchSchemas({
          ...
          subschemas: [
            {
              schema: buildSchema(
                'type Query { field(status: Status): Int } enum Status { YES }',
              ),
            },
            {
              schema: buildSchema(
                'type Query { field(status: Status): Int } enum Status { NO }',
              ),
            },
          ],
        });
