...
export function validateFieldConsistency<TContext = Record<string, any>>(
  ...
  if (
    getValidationSettings(fieldNamespace, typeMergingOptions)
    ...
    finalFieldNull &&
    candidates.some((c) => !isNonNullType(c.fieldConfig.type))
  ) {
    validationMessage(
  ...
  const argCandidatesMap: Record<
    string,
    Array<GraphQLArgumentConfig>
  > = Object.create(null);
  ...
  for (const argName in argCandidatesMap) {
    ...
    if (
      getValidationSettings(argNamespace, typeMergingOptions)
        .strictNullComparison &&
      ...
      !finalArgNull &&
      argCandidates.some((c) => isNonNullType(c.type))
    ) {
      validationMessage(
...
export function validateInputFieldConsistency<TContext = Record<string, any>>(
  ...
  if (
    getValidationSettings(inputFieldNamespace, typeMergingOptions)
      .strictNullComparison &&
    candidates.some(
    ...
    !finalInputFieldNull &&
    candidates.some((c) => isNonNullType(c.inputFieldConfig.type))
  ) {
    validationMessage(
...
export function validateTypeConsistency<TContext = Record<string, any>>(
  ...
  for (const c of candidates) {
    ...
    if (finalNamedType.toString() !== currentNamedType.toString()) {
      const proxiableScalar =
        !!typeMergingOptions?.validationSettings?.proxiableScalars?.[
...
export function validateInputEnumConsistency<TContext = Record<string, any>>(
  ...
  if (
    Object.values(enumValueInclusionMap).some(
