...
import { stringifyString } from './data.js';
...
export interface ProjectionPlanField {
  ...
  isTypename: ReadonlySet<string> | false;
  /**
...
const planCache = new WeakMap<
  GraphQLSchema,
  WeakMap<OperationDefinitionNode, CompiledProjectionPlan | null>
>();
...
export function getOrCompileProjectionPlan(
  ...
  let byOp = planCache.get(executionContext.supergraphSchema);
  if (!byOp) {
    byOp = new WeakMap();
    planCache.set(executionContext.supergraphSchema, byOp);
  }
  let plan = byOp.get(executionContext.operation);
  if (!plan) {
    plan = buildProjectionPlan(executionContext);
    byOp.set(executionContext.operation, plan);
  }
  return plan;
}
 ...
 */
function getScalarHint(
  type: GraphQLScalarType,
): 'string' | 'number' | 'boolean' | 'id' | null {
  switch (type.name) {
    case 'String':
      return 'string';
    case 'Int':
...
function unionTypenameGuards(
  a: ReadonlySet<string> | false,
  b: ReadonlySet<string> | false,
    ...
    for (const d of directives) {
      ...
      if (d.name.value === 'skip') {
        if (ifArg.value.kind === Kind.VARIABLE) {
          (skipIfVars ??= []).push(
            (ifArg.value as { name: { value: string } }).name.value,
          );
        } else if (
          ifArg.value.kind === Kind.BOOLEAN &&
          ifArg.value.value === true
        ) {
          literalSkip = true;
        }
    ...
    switch (selection.kind) {
      case Kind.FIELD: {
        ...
        if (fieldName === TYPENAME) {
          const escapedKey = stringifyString(responseKey) + ':';
          const hasSkip = skipIfVars.length > 0;
          const hasInclude = includeIfVars.length > 0;
          const isTypenameGuards = getPossibleTypeNames(schema, parentTypeName);
          const field: ProjectionPlanField = {
            responseKey,
            escapedKey,
            commaEscapedKey: ',' + escapedKey,
            isTypename: isTypenameGuards,
            skipIfVars,
            includeIfVars,
            hasSkip,
            hasInclude,
          ...
          fields.set(
            responseKey,
            existing ? mergeField(existing, field) : field,
          );
          break;
        }
        ...
        const enumValues: ReadonlySet<string> | null = isEnumType(fieldType)
          ? new Set(
              (fieldType as GraphQLEnumType).getValues().map((v) => v.name),
            )
          : null;
      ...
      case Kind.INLINE_FRAGMENT: {
        ...
        if (selection.typeCondition) {
          fragmentTypeGuard = getPossibleTypeNames(
            schema,
        ...
        const effectiveGuard = intersectGuards(
          parentTypeGuard,
          fragmentTypeGuard,
        );
