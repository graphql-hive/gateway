...
export function createGraphQLThriftClient<Context = any>({
  ...
  class GraphQLThriftClient extends ThriftClient<Context> {
    ...
    public static override readonly annotations: IThriftAnnotations =
      annotations;
    public static override readonly methodAnnotations: IMethodAnnotations =
      methodAnnotations;
    ...
    public override readonly _annotations: IThriftAnnotations = annotations;
    public override readonly _methodAnnotations: IMethodAnnotations =
      methodAnnotations;
    public override readonly _methodNames: Array<string> = methodNames;
    public readonly _methodParameters?: Record<string, number> =
      methodParameters;
      ...
      switch (typeVal.type) {
        ...
        case TType.LIST:
          output.writeListBegin(
            getActualType(typeVal.elementType),
            value.length,
          );
    ...
    async doRequest(
      ...
      const { fieldName, messageType }: IThriftMessage =
        input.readMessageBegin();
      if (fieldName === methodName) {
        if (messageType === MessageType.EXCEPTION) {
          const err: TApplicationException =
            TApplicationExceptionCodec.decode(input);
  ...
  // @ts-expect-error TODO: fix this weirdness (or not weird?)
