...
export function getThriftExecutor(subgraph: GraphQLSchema): Executor {
  ...
  return async function thriftExecutor(
    ...
    const operationsAndFragments = getOperationsAndFragments(
      executionRequest.document,
    );
    const operationName =
      executionRequest.operationName ||
      Object.keys(operationsAndFragments.operations)[0]!;
    const operationAst = operationsAndFragments.operations[operationName]!;
    const rootType = rootTypeMap.get(operationAst.operation);
    if (!rootType) {
      throw new Error(
        `No root type found for operation type ${operationAst.operation}`,
      );
    }
    const rootFieldMap = rootType.getFields();
    const rootFieldsWithArgs = getRootFieldsWithArgs(
      subgraph,
      executionRequest,
    );
    ...
    for (const [fieldName, args] of rootFieldsWithArgs) {
      ...
      if (fieldTypeMapDirectives == null) {
        fieldTypeMapDirectives =
          getDirectiveExtensions(field)?.['fieldTypeMap'] || [];
          ...
          client
            .doRequest(fieldName, args, fieldTypeMapDirective?.fieldTypeMap, {
              headers: headersFactory({
                ...
                env: Object(globalThis.process?.env),
              }),
            })
            .then((result) => {
