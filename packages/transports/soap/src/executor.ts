...
const defaultFieldResolver: GraphQLFieldResolver<any, any> =
  function soapDefaultResolver(root, args, context, info) {
...
Falling back to 'http://www.w3.org/2003/05/soap-envelope' as SOAP Namespace.`);
  ...
  return async function rootValueMethod(
    args: any,
    context: any,
    info: GraphQLResolveInfo,
  ) {
    ...
    envelopeAttributes[`xmlns:${bodyPrefix}`] =
      soapAnnotations.bindingNamespace;

    const headerPrefix =
      soapAnnotations.soapHeaders?.alias ||
      soapAnnotations.bodyAlias ||
      'header';
    if (soapAnnotations.soapHeaders?.headers) {
      ...
      if (soapAnnotations.soapHeaders?.namespace) {
        envelopeAttributes[`xmlns:${headerPrefix}`] =
          soapAnnotations.soapHeaders.namespace;
      }
    }
    ...
    try {
      const responseJSON = xmlToJSONConverter.parse(
        responseXML,
        parseXmlOptions,
      );
      return normalizeResult(
        responseJSON.Envelope[0].Body[0][soapAnnotations.elementName],
      );
    } catch (e) {
      return createGraphQLError(`Invalid SOAP response: ${String(e)}`, {
        extensions: {
...
function createRootValue(
  ...
  const operationHeadersFactory =
    getInterpolatedHeadersFactory(operationHeaders);
