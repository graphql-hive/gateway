...
function filterIntrospectionDefinitions<
  TASTNode extends { directives?: readonly DirectiveNode[] },
>(node: TASTNode): TASTNode | null {
  if (
...
export async function getNeo4JExecutor(
  opts: Neo4JExecutorOpts,
...
export function getExecutableSchemaFromTypeDefsAndDriver({
  ...
  if (pubsub) {
    features = {
      subscriptions: {
        ...
        publish: (eventMeta: any) => pubsub.publish(eventMeta.event, eventMeta),
        close: () => {},
      },
    };
  }
  ...
  const neo4jGraphQL = new Neo4jGraphQL({
    // @ts-expect-error these are type definitions
