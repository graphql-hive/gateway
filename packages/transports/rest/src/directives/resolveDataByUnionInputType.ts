  ...
  if (data) {
    if (isListType(type)) {
      return asArray(data).map((elem) =>
        resolveDataByUnionInputType(elem, type.ofType, schema),
      );
    }
    if (type && isNonNullType(type)) {
    ...
    if (isInputObjectType(type)) {
      ...
      for (const propertyName in data) {
        ...
        if (field) {
          ...
          const fieldResolveRootFieldDirectives = getDirective(
            schema,
            field,
            'resolveRootField',
          );
          const realFieldName =
            fieldResolveRootFieldDirectives?.[0]?.['field'] || fieldName;
