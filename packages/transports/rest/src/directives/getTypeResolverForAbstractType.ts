...
export function getTypeResolverForAbstractType({
  ...
  return function resolveType(data: any, _ctx: any, info: GraphQLResolveInfo) {
    if (data.__typename) {
      ...
      return (
        discriminatorMappingObj?.[discriminatorValue] || discriminatorValue
      );
    }
    if (data.$statusCode && statusCodeTypeNameMap) {
      const typeName =
        statusCodeTypeNameMap[data.$statusCode.toString()] ||
        statusCodeTypeNameMap['default'];
      if (typeName) {
    ...
    if (data.$response) {
      const error = createGraphQLError(
        `Upstream HTTP Error: ${data.$statusCode}`,
        {
          extensions: {
