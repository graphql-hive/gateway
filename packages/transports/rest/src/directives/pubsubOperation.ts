...
export function processPubSubOperationAnnotations({
  ...
  field.subscribe = function pubSubSubscribeFn(root, args, context, info) {
    ...
    const operationLogger = logger.child({
      operation: `${info.parentType.name}.${field.name}`,
    });
    ...
    let interpolatedPubSubTopic: string = stringInterpolator.parse(
      pubsubTopic,
      interpolationData,
    );
    if (interpolatedPubSubTopic.startsWith('webhook:')) {
      const [, expectedMethod, expectedUrl] =
        interpolatedPubSubTopic.split(':');
      const expectedPath = new URL(expectedUrl || '', 'http://localhost')
        .pathname;
  ...
  field.resolve = function pubSubResolver(root, _args, context, info) {
    const logger = context?.logger || globalLogger;
    const operationLogger = logger.child({
      operation: `${info.parentType.name}.${field.name}`,
    });
