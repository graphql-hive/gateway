...
export function processLinkFieldAnnotations(
  ...
  field.resolve = function linkDirectiveHandler(root, args, context, info) {
    ...
    return linkResolver(linkResolverOpts, {
      root,
      args,
      context,
      info,
...
function findLinkResolverMap({
  ...
  const defaultRootType = schema.getType(
    defaultRootTypeName,
...
function getLinkResolverMap(
  schema: GraphQLSchema,
  field: GraphQLField<any, any>,
) {
  const parentFieldLinkResolverDirectives = getDirective(
    schema,
    field,
    'linkResolver',
  );
  if (parentFieldLinkResolverDirectives?.length) {
    const linkResolverMap =
      parentFieldLinkResolverDirectives[0]?.['linkResolverMap'];
    if (linkResolverMap) {
