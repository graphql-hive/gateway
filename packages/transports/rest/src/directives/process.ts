...
export function processDirectives(
  ...
  if (Array.isArray(globalOptions.operationHeaders)) {
    globalOptions.operationHeaders = Object.fromEntries(
      globalOptions.operationHeaders,
    );
  }
  ...
  for (const typeName in typeMap) {
        ...
        switch (directiveAnnotation.name) {
          case 'statusCodeTypeName':
            ...
            statusCodeTypeNameIndexMap[
              directiveAnnotation.args?.['statusCode']
    ...
    if (isEnumType(type)) {
      ...
      for (const directiveAnnotation of directiveAnnotations) {
        switch (directiveAnnotation.name) {
          case 'typescript':
            processTypeScriptAnnotations(
              type,
              directiveAnnotation.args?.['type'],
            );
            break;
        }
      }
        ...
        for (const directiveAnnotation of directiveAnnotations) {
          switch (directiveAnnotation.name) {
            ...
            case 'pubsubOperation':
              ...
              break;
            case 'httpOperation': {
              let operationSpecificHeaders =
                directiveAnnotation.args?.['operationSpecificHeaders'];
              if (
                typeof directiveAnnotation.args?.[
              ...
              addHTTPRootFieldResolver(
                ...
                // TODO: Fix JSON parsing here for queryParams and headers
                {
                  ...
                  requestBaseBody:
                    typeof directiveAnnotation.args?.['requestBaseBody'] ===
                    'string'
                      ? JSON.parse(directiveAnnotation.args['requestBaseBody'])
                      : directiveAnnotation.args?.['requestBaseBody'],
                  queryParamArgMap:
                    typeof directiveAnnotation.args?.['queryParamArgMap'] ===
                    'string'
                      ? JSON.parse(
                  ...
                  queryStringOptions:
                    directiveAnnotation.args?.['queryStringOptions'],
                },
            ...
            case 'responseMetadata':
              processResponseMetadataAnnotations(
                field as GraphQLField<any, any>,
              );
              break;
            case 'oas_link':
