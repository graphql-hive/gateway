...
const isListTypeOrNonNullListType = memoize1(
...
export function addHTTPRootFieldResolver(
  ...
  field.resolve = async (root, args, context, info) => {
    ...
    const interpolatedBaseUrl = stringInterpolator.parse(
      endpoint,
      interpolationData,
    );
      ...
      if (requestBaseBody != null) {
        ...
        for (const key in requestBaseBody) {
          ...
          if (typeof configValue === 'string') {
            const value = stringInterpolator.parse(
              configValue,
              interpolationData,
            );
            ...
            if (inputValue != null) {
              ...
              if (typeof inputValue === 'object') {
                if (inputValue instanceof File) {
                  ...
                  formDataValue = new File(
                    [inputValue],
                    (inputValue as File).name,
                    {
                  ...
                  if (inputValue.name) {
                    ...
                    formDataValue = new Blob([arrayBuffer], {
                      type: inputValue.type,
                    });
                  }
                } else {
    ...
    const fetch: typeof globalFetch =
      context?.fetch || globalFetch || defaultFetch;
    ...
    try {
      ...
      if (isScalarType(returnNamedGraphQLType)) {
        operationLogger.debug(
          ` => Return type is not a JSON so returning ${responseText}`,
        );
        return responseText;
      } else if (
        response.status === 204 ||
