...
describe('grpc utils', () => {
  describe('addMetaDataToCall', () => {
    const grpcClientMethod = vi.fn();
    const input = { sport: 'Baseball' };
    const context = {
      team: 'Oakland As',
      players: { pitcher: 'Kershaw' },
      number: 42,
    };
    ...
    function createExpectedMetadata(
      key: string,
      value: string | Buffer,
    ): Metadata {
    ...
    it(`when no metadata is supplied by the config`, () => {
      addMetaDataToCall(
        grpcClientMethod,
        input,
        { context, env: Object(process.env) },
        undefined,
      );
    ...
    describe.each<
      [string, Record<string, string | Buffer | string[]>, Metadata]
    >([
      [
        'static',
        { sportsTeam: 'Dodgers' },
      ...
      it(`when ${type} metadata is supplied by the config`, () => {
        addMetaDataToCall(
          grpcClientMethod,
          input,
          { context, env: Object(process.env) },
