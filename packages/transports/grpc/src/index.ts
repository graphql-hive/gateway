...
export class GrpcTransportHelper extends DisposableStack {
  ...
  getCredentials(): MaybePromise<ChannelCredentials> {
    ...
    if (this.config.credentialsSsl) {
      const credentialsSsl = this.config.credentialsSsl;
      this.logger.debug(
        () =>
          `Using SSL Connection with credentials at ${credentialsSsl.privateKey} & ${credentialsSsl.certChain}`,
      );
      const absolutePrivateKeyPath = path.isAbsolute(credentialsSsl.privateKey)
        ? credentialsSsl.privateKey
        : path.join(this.baseDir, credentialsSsl.privateKey);
      const absoluteCertChainPath = path.isAbsolute(credentialsSsl.certChain)
        ? credentialsSsl.certChain
        : path.join(this.baseDir, credentialsSsl.certChain);

      const sslFiles = [
        fs.promises.readFile(absolutePrivateKeyPath),
        fs.promises.readFile(absoluteCertChainPath),
      ];
      if (credentialsSsl.rootCA !== 'rootCA') {
        const absoluteRootCAPath = path.isAbsolute(credentialsSsl.rootCA)
          ? credentialsSsl.rootCA
          : path.join(this.baseDir, credentialsSsl.rootCA);
  ...
  getServiceClient({
    ...
    let serviceClientByObjPath =
      this.grpcObjectByserviceClientByObjPath.get(grpcObject);
    if (!serviceClientByObjPath) {
      serviceClientByObjPath = new Map<string, ServiceClient>();
      this.grpcObjectByserviceClientByObjPath.set(
        grpcObject,
        serviceClientByObjPath,
      );
    }
    ...
    if (!client) {
      ...
      if (typeof ServiceClient !== 'function') {
        throw new Error(
          `Object at path ${objPath} is not a Service constructor`,
        );
      }
  ...
  processDirectives({
    ...
    if ('ObjMap' in schemaTypeMap) {
      addExecutionLogicToScalar(
        schemaTypeMap['ObjMap'] as GraphQLScalarType,
        ObjMapScalar,
      );
    }
  ...
  getSubgraphExecutor({ transportEntry, subgraph, cwd, logger }) {
    ...
    return handleMaybePromise(
      ...
      (creds) => {
        ...
        return makeDisposable(createDefaultExecutor(subgraph), () =>
          transport.dispose(),
        );
      },
    );
