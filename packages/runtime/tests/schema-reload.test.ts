...
import { composeLocalSchemasWithApollo, createDisposableServer } from "@internal/testing";
...
describe('Schema reload', () => {
    ...
    it('retries automatically when the schema reloads', async () => {
        ...
        await using firstUpstreamServer = await createDisposableServer(firstUpstreamYoga);
        ...
        await using secondUpstreamYoga = createYoga({ schema: secondUpstreamSchema });
        await using secondUpstreamServer = await createDisposableServer(secondUpstreamYoga);
        let fetchCount = 0;
