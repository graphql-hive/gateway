// Step 2: Add MCP with one inline tool.
//
// Adds the useMCP plugin with a single tool whose GraphQL query is defined inline.
// The gateway now exposes an /mcp endpoint that speaks the MCP protocol.
// AI agents can discover tools via tools/list and call them via tools/call.
//

// alias toolsList='curl -s http://localhost:4000/mcp -H "Content-Type: application/json" -d '\''{"jsonrpc":"2.0","id":1,"method":"tools/list","params":{}}'\'' | jq ".result.tools[] | {name, description}"'
...
// NEW: Add MCP with a single inline tool
const mcpOptions = {
  name: 'weather-api',
  version: '1.0.0',
  tools: [
...
const gateway = createGatewayRuntime({
  ...
  plugins: (ctx) => [useMCP(ctx, mcpOptions)], // <----- Add the MCP plugin to the gateway
});
