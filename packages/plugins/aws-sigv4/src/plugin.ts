...
const DEFAULT_INCOMING_OPTIONS: Required<AWSSignv4PluginIncomingOptions> = {
  ...
  secretKey: () => process.env['AWS_SECRET_ACCESS_KEY'] || process.env['AWS_SECRET_KEY'],
  onExpired() {
...
export function useAWSSigv4<TContext extends Record<string, any>>(
  ...
  const incomingOptions: Required<AWSSignv4PluginIncomingOptions> | undefined =
    opts.incoming != null && opts.incoming !== false
      ? opts.incoming === true ? DEFAULT_INCOMING_OPTIONS : { ...DEFAULT_INCOMING_OPTIONS, ...opts.incoming }
      : undefined;
  return {
          ...
          return handleMaybePromise(() => {
            ...
            return handleMaybePromise(
              ...
              (ifContinue) => {
                ...
                const xAmzDate = request.headers.get(AWSSignV4Headers.XAmzDate);
                ...
                if (!xAmzExpires) {
                  ...
                  localDateTime.setSeconds(localDateTime.getSeconds(), xAmzExpires);
                ...
                return handleMaybePromise(() => incomingOptions.secretKey(payload), (secretKey) => {
                  if (!secretKey) {
                    return incomingOptions.onSignatureMismatch?.(request, serverContext);
                  }
                  payload.secretKey = secretKey;
                  return handleMaybePromise(() => incomingOptions.onAfterParse(payload), shouldContinue => {
                    if (!shouldContinue) {
                      return;
                    }
                    return payload;
            ...
            const credentialString = [payload?.date, payload?.region, payload?.service, payload?.requestType].join('/');
    ...
    onFetch({ url, options, setURL, setOptions, executionRequest }) {
      ...
      return handleMaybePromise(() => (roleArn && roleSessionName) ? new STS({ region }).assumeRole({
        RoleArn: roleArn,
