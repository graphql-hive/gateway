...
Tools can also be auto-registered from `.graphql` files using `@mcpTool` directives, or fetched from Hive App Deployments:
...
- **Tool sources** - inline queries, named operations from files, `@mcpTool` directives, Hive App Deployments
...
- **Resources** - static docs (inline text or file) and parameterized resource templates
