// Step 7: Override tool descriptions and input schema fields.
//
// Use the 'tool' and 'input' config sections to customize how tools appear to AI agents. 
// Override titles, descriptions, and add examples or descriptions to individual input fields, merged on top of the auto-derived schema.
//
// curl -s http://localhost:4000/mcp -H "Content-Type: application/json" -d '{"jsonrpc":"2.0","id":1,"method":"tools/list","params":{}}' | jq '.result.tools[] | select(.name == "get_weather")'
//
  ...
  tools: [
    {
      ...
      // NEW: Tool-level overrides
      tool: {
        title: 'Current Weather',
        description: 'Get the current weather for a city',
      },
      // NEW: Field-level input schema overrides (merged with auto-derived schema)
      input: {
        schema: {
          properties: {
