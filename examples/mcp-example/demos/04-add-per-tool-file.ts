// Step 4: Per-tool file override. A tool can point to its own .graphql file.
//
...
//
// curl -s http://localhost:4000/mcp -H "Content-Type: application/json" -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"get_conditions","arguments":{"location":"London"}}}' | jq '.result.structuredContent'
...
const mcpPlugin = useMCP({
  ...
  tools: [
    ...
    // NEW: Per-tool file override, this tool loads from its own .graphql file instead of the shared operationsPath
    {
      name: 'get_conditions',
      source: {
        type: 'graphql',
