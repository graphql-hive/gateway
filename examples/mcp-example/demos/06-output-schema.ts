// Step 6: Output schema, auto-generated from the GraphQL selection set.
//
// Each tool automatically gets an outputSchema derived from the GraphQL query's selection set. 
// No config changes needed, the plugin introspects the schemaand generates JSON Schema for the response shape.
//
// curl -s http://localhost:4000/mcp -H "Content-Type: application/json" -d '{"jsonrpc":"2.0","id":1,"method":"tools/list","params":{}}' | jq '.result.tools[] | select(.name == "get_weather") | .outputSchema'
//
// curl -s http://localhost:4000/mcp -H "Content-Type: application/json" -d '{"jsonrpc":"2.0","id":1,"method":"tools/list","params":{}}' | jq '.result.tools[] | select(.name == "get_forecast") | .outputSchema'
...
// Same config as step 5. Each tool gets an outputSchema derived from the selection set.
const mcpOptions = {
  name: 'weather-api',
  version: '1.0.0',
  ...
  tools: [
    ...
    },
    {
      ...
      source: {
        ...
        operationName: 'GetForecast',
        operationType: 'query',
        file: join(exampleDir, 'operations/weather_override.graphql'),
      },
    },
  ],
}
