...
export const sprintf = (fmt: string, ...args: unknown[]): string => {
  ...
  return fmt.replaceAll(
    RE,
    (_: string, index?: string, precision?: string, specifier?: string) => {
      if (specifier === '%') {
        return '%';
      }
      if (index !== undefined) {
...
export const findIndexesOfSubString = (
  inputString: string,
  searchString: string,
): number[] => {
  const matches = [];
...
export const caseInsensetiveComparator = function (
  a: string,
  b: string,
): number {
  a = a.toUpperCase();
...
export const trimEndWithMaxLength = (
  str: string,
  maxLength: number,
): string => {
  if (str.length <= maxLength) {
...
export const createPlainTextSearchRegex = function (
  query: string,
  flags?: string,
): RegExp {
...
export const toKebabCase = function (input: string): Lowercase<string> {
  return (input
    ...
    ?.map((w) => w.toLowerCase())
    .join('-')
    .replaceAll('-.-', '.') || input) as Lowercase<string>;
};

/* eslint-disable @typescript-eslint/no-explicit-any */
export function toKebabCaseKeys(settingValue: { [x: string]: any }): {
  [x: string]: any;
} {
  const result: {
    [x: string]: any;
...
export const replaceLast = function (
  input: string,
  search: string,
  replacement: string,
): string {
