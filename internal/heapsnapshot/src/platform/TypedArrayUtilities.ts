 ...
 */
export function createFixedBigUint32Array(
  length: number,
  maxLengthForTesting?: number,
): BigUint32Array {
  try {
...
class SplitBigUint32ArrayImpl implements BigUint32Array {
  ...
  constructor(length: number, maxLengthForTesting?: number) {
    ...
    while (true) {
      ...
      try {
        if (
          maxLengthForTesting !== undefined &&
          this.#partLength > maxLengthForTesting
        ) {
          // Simulate allocation failure.
          throw new RangeError();
        }
...
class ExpandableBigUint32ArrayImpl
  extends Array<number>
  implements BigUint32Array
{
  getValue(index: number): number {
    return this[index];
  }
...
export function createBitVector(
  lengthOrBuffer: number | ArrayBuffer,
): BitVector {
