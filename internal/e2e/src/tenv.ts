...
import { truthyEnv } from '~internal/env';
...
`);
  ...
  if (runner === 'bun-docker' && !truthyEnv('CI')) {
    process.stderr.write(`
⚠️ Using docker gateway runner! Make sure you have built the containers with:
...
export const dockerHostName = truthyEnv('CI')
  ? '172.17.0.1'
  ...
  const tenv: Tenv = {
    ...
    async gateway(opts) {
      let {
        ...
        pipeLogs = isDebug() || truthyEnv('E2E_PIPE_LOGS')
          ? 'gateway.out'
          : false,
        env,
        runner,
    ...
    async composeWithMesh(opts) {
      const {
        ...
        trimHostPaths,
        maskServicePorts,
        pipeLogs = isDebug() || truthyEnv('E2E_PIPE_LOGS') ? 'mesh.out' : false,
        env,
        args = [],
      } = opts || {};
      let output = '';
    ...
    async composeWithApollo({
      services = [],
      pipeLogs = isDebug() || truthyEnv('E2E_PIPE_LOGS') ? 'rover.out' : false,
    }) {
