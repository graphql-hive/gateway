...
describe.skipIf(usingHiveRouterRuntime())('edfs-subgraph-defs', () => {
  const natsEnv = {
    NATS_HOST: '',
    NATS_PORT: 0,
  };
  ...
  it(
    'should receive subscription published event on all distributed gateways',
    async () => {
  ...
  it(
    'should send a payload from a mutation to another gateway using NATS',
    async () => {
      const products = await service('products');
