...
import { isCI, usingHiveRouterRuntime } from '~internal/env';
...
describe.skipIf(
  ...
  it.skipIf(
    // hive router runtime wont even reach the execution because it'll fail on
    // parsing during query planning
    usingHiveRouterRuntime(),
  )(
    'usage reporting of subscriptions must not have empty fields when genericAuth strips them',
    async () => {
      const users = await service('users');
      const supergraph = await composeWithApollo({
        services: [await service('posts'), users],
