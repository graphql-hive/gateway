...
it('should subscribe and cancel', async () => {
  ...
  expect(gw.getStd('out').replaceAll(' ', '')).toContain('ITERABLE>NEXT>END'); // TODO: there shouldnt be multiple "END"s
  expect(srv.getStd('out').replaceAll(' ', '')).toBe('ITERABLE>NEXT>END');
});
